...
<body>
	...
	%>	
		<tr>
			...
			<%int idx=BoardDAO.getInstance().getIdx(list.get(i).getNo()); %>
				<button onclick="window.location.href='_06_deleteBoardPro.jsp?delNo=<%=BoardDAO.getInstance().getIdx(list.get(i).getNo())%>&pg=<%=i/5%>'">삭제하기</button>
			</td>
		</tr>
	...
	<%  if(start > pageLimit) { %>             <!--  4-1 : 3   -->  <!-- 계속 3으로 유지 4-3 1   -->
		[<a href="_08_boardListPaging.jsp?start=<%= start - 1 %>&end=<%= start - pageLimit %>">이전</a>]
	<%	} %>
	<!-- 1 2 3 -->  <!-- 4 5 6  -->  <!-- end는 페이지 끝 번호  -->
	<%  for(int i=start; i<=end; i++) { %>
			[<a href="_08_boardListPaging.jsp?start=<%= i %>&end=<%= start %>"><%= i %></a>]
	<%	} %>
	
	<%  if(end < totalPageNum) { %>
		[<a href="_08_boardListPaging.jsp?start=<%= start + pageLimit %>&end=<%= start + pageLimit%>">이후</a>]
	<%	} %>
</body>
</html>